...
--
-- Host: 127.0.0.1
-- Generation Time: May 04, 2019 at 08:21 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetActivityLogs` (IN `p_account_id` INT, IN `p_activity_from` INT, IN `p_activity_to` INT)  NO SQL
BEGIN

    CREATE TEMPORARY TABLE tempActivityLogs (
        row_num int,
        activity_log_id int,
        account_id int,
    ...
    insert into tempActivityLogs
	Select (@row_number:=@row_number + 1) AS row_num, a.* from activity_logs a where account_id = p_account_id order by activity_log_id desc;
    
    if p_activity_from = 0 and p_activity_to = 0 then
    	
    		select * from tempActivityLogs;
        
	else
    	
        	select * from tempActivityLogs where row_num >= p_activity_from and row_num <= p_activity_to;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetActivityLogsAbsoluteCount` (IN `p_account_id` INT)  NO SQL
begin 

	select count(*) as absoluteCount from activity_logs where account_id = p_account_id;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCurriculumDetails` (IN `param_curriculum_year` INT, IN `param_course_id` INT)  NO SQL
begin

	select cs.description, c.curriculum_year from 
    curriculum c inner join courses cs 
    on c.course_id = cs.course_id 
    where 
    	c.curriculum_year = param_curriculum_year and 
        cs.course_id = param_course_id
     limit 1;
    
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCurriculumSubjectDetails` (IN `param_curriculum_year` INT, IN `param_course_id` INT)  NO SQL
begin

    select 
        sms.semester_id,
        concat(sms.semester,' ',' Semester') as semester,
        cys.year_level,
        cs.description,
        s.subject_code,
        ...
        s.lec_units,
        s.lab_units,
        (s.lec_units + s.lab_units) as 'total_units' 
    from 
        curriculum c 
        inner join courses cs on c.course_id = cs.course_id
        inner join curriculum_year_sem cys on c.curriculum_id = 		cys.curriculum_id
        inner join curriculum_subjects css on cys.curriculum_year_sem_id = css.curriculum_year_sem_id
        inner join semesters sms on cys.semester_id = sms.semester_id
        inner join subjects s on css.subject_id = s.subject_id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetNotifications` (IN `p_account_id` INT, IN `p_notification_from` INT, IN `p_notification_to` INT)  NO SQL
BEGIN

    CREATE TEMPORARY TABLE tempNotifications (
        row_num int,
        notification_id int,
        notification_from int,        
        notification_from_user_type text,
        notification_from_user_fullname text,
        notification_from_user_image text,
        notification_to int,
        description text,
        path text,
        datetime datetime,
    ...
    from (
        select 
        	...
        	concat(first_name, ' ', last_name) as userfullname,
        	image as userimage
        FROM teachers
    ...
    ) as u inner join accounts a 
	on u.user_id = a.user_id and u.user_type = a.type;
	...
	Select 
        ...
        n.notification_id,
        n.notification_from,        
        u.user_type as notification_from_user_type,
        u.userfullname as notification_from_user_fullname,
        u.userimage as notification_from_user_image,
        n.notification_to,
        n.description,
        n.path,
        n.datetime,
    ...
    inner join userList u
        on n.notification_from = u.account_id and n.notification_from_user_type = u.user_type
    where n.notification_to = p_account_id order by n.notification_id desc;

    if p_notification_from = 0 and p_notification_to = 0 then
    	
    		select * from tempNotifications;
        
	else
    	
        	select * from tempNotifications where row_num >= p_notification_from and row_num <= p_notification_to;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetNotificationsAbsoluteCount` (IN `p_account_id` INT)  NO SQL
begin 

	select count(*) as absoluteCount from notifications where notification_to = p_account_id;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetUnreadNotifications` (IN `p_account_id` INT)  NO SQL
begin
	select count(*) as unreadNotificationsCount from notifications where notification_to = p_account_id and is_read = 0;
...
(1, 1, 'admin', 'admin', 'Starta123', 'vanessatali@gmail.com', '2b0cd5f075d9b03dfbd66f59e2d43c6e91ce65ec', '2017-02-03 06:48:54', '2019-03-23 06:32:07', 1),
...
(7, 5, 'teacher', 'sam', 'Starta123', 'evilking@gmail.com', '2b0cd5f075d9b03dfbd66f59e2d43c6e91ce65ec', '2017-02-20 18:40:11', '2019-03-24 06:53:49', 1),
...
(24, 22, 'student', 'myra', 'roi', 'myra@gmail.com', '70fe19c13a4007bd60b3b30ca92b8c191f0ba12e', '2017-04-16 16:04:13', '2019-03-10 07:33:39', 1),
...
(29, 10, 'teacher', 'roilarrence.amatong', 'Starta123', 'amatongroi@gmail.com', '2b0cd5f075d9b03dfbd66f59e2d43c6e91ce65ec', '2018-12-31 21:42:37', '2019-03-10 08:11:47', 1),
...
(35, 30, 'student', 'myra', '123', 'myra@gmail.com', '40bd001563085fc35165329ea1ff5c5ecbdbbeef', '2019-03-03 16:36:28', NULL, 1),
(36, 31, 'student', 'athrun', 'Starta123', 'athrunzala@gmail.com', '2b0cd5f075d9b03dfbd66f59e2d43c6e91ce65ec', '2019-03-10 07:24:12', NULL, 1),
(37, 13, 'teacher', 'marru.ramius', 'Starta123', 'marru@gmail.com', '2b0cd5f075d9b03dfbd66f59e2d43c6e91ce65ec', '2019-03-10 07:44:28', NULL, 1);
...
CREATE TABLE `activity_logs` (
  ...
  `acitivity_datetime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `activity_logs` (`activity_log_id`, `account_id`, `activity_description`, `acitivity_datetime`) VALUES
(65, 7, 'Post: Created new post.', '2019-03-23 18:09:17'),
(66, 7, 'Post: Update post', '2019-03-23 18:13:30'),
(67, 7, 'Post: Delete post.', '2019-03-23 18:26:04'),
(68, 7, 'Class List: Unenroll student \'Mark Dale O. Kwan\'.', '2019-03-23 18:31:30'),
(69, 7, 'Class List: Enrolled student \'Mark Dale O. Kwan\'.', '2019-03-24 12:27:05'),
(70, 7, 'Class List: Unenrolled student \'Samuel R. Greyjoy\'.', '2019-03-24 12:27:32'),
(71, 7, 'Homework: Created new homework \'Fundamentals of Programming\'.', '2019-03-24 13:19:19'),
(72, 7, 'Homework: Updated homework \'Fundamentals of Programming - updated\'.', '2019-03-24 14:15:34'),
(73, 7, 'Homework: Unsend homework \'0\'.', '2019-03-24 14:39:55'),
(74, 7, 'Homework: Unsend homework \'0\'.', '2019-03-24 14:41:28'),
(75, 7, 'Homework: Send homework \'Fundamentals of Programming - updated\'.', '2019-03-24 14:43:34'),
(76, 7, 'Homework: Unsend homework \'Fundamentals of Programming - updated\'.', '2019-03-24 14:43:53'),
(77, 7, 'Homework: Unsend homework \'(2) Homework - updated\'.', '2019-03-24 14:44:12'),
(78, 7, 'Profile: Updated profile picture.', '2019-03-24 14:49:53'),
(79, 7, 'Profile: Updated personal information.', '2019-03-24 14:51:55'),
(80, 7, 'Profile: Updated account information.', '2019-03-24 14:53:53'),
(81, 32, 'Post: Created a post.', '2019-03-24 15:07:22'),
(82, 32, 'Post: Updated post.', '2019-03-24 19:28:44'),
(83, 32, 'Post: Deleted post.', '2019-03-24 19:32:34'),
(84, 32, 'Homework: Submitted file on homework \'Fundamentals of Programming - updated\'.', '2019-03-24 19:41:01'),
(85, 1, 'Faculty Load: Assigned new subject CSST145-Thesis 1.5 (1st year) to TID- 20488989 - Sam L. Smith', '2019-03-27 13:21:28'),
(86, 1, 'Faculty Load: Remove subject CSST145-Thesis 1.5 which is assigned to TID- 20488989 - Sam L. Smith.', '2019-03-27 13:22:29'),
(87, 1, 'Faculty Load: Remove subject CSST145-Thesis 1.5 which is assigned to TID- 20488989 - Sam L. Smith.', '2019-03-27 13:22:42'),
(88, 1, 'Faculty Load: Assigned new subject CSST145-Thesis 1.5 (1st year) to TID- 20488989 - Sam L. Smith', '2019-03-27 13:23:18'),
(89, 1, 'Faculty Load: Remove subject CSST145-Thesis 1.5 which is assigned to TID- 20488989 - Sam L. Smith.', '2019-03-27 13:23:58'),
(90, 1, 'Faculty Load: Assigned new subject CSST145-Thesis 1.5 (1st year) to TID- 20488989 - Sam L. Smith', '2019-03-27 13:24:58'),
(91, 1, 'Faculty Load: Remove subject CSST145-Thesis 1.5 which is assigned to TID- 20488989 - Sam L. Smith.', '2019-03-28 11:44:08'),
(92, 1, 'Faculty Load: Assigned new subject CSST145-Thesis 1.5 (1st year) to TID- 20488989 - Sam L. Smith', '2019-03-28 11:59:45'),
(93, 1, 'Faculty Load: Remove subject CSST145-Thesis 1.5 which is assigned to TID- 20488989 - Sam L. Smith.', '2019-03-28 12:37:11'),
(94, 1, 'Class: Assign student (SM2012-018080) Samuel R. Greyjoy to class CSST1222-Advanced Database System (1st year)', '2019-03-28 12:46:38'),
(95, 1, 'Class: Remove student (SM2012-00122) Mark Dale O. Kwan from class CSST1222-Advanced Database System (1st year)', '2019-03-28 12:54:29'),
(96, 7, 'Homework: Send homework \'Fundamentals of Programming - updated\'.', '2019-03-28 13:18:33'),
(97, 7, 'Homework: Unsend homework \'Fundamentals of Programming - updated\'.', '2019-03-28 13:19:42'),
(98, 7, 'Homework: Send homework \'Fundamentals of Programming - updated\'.', '2019-03-28 13:20:16'),
(99, 7, 'Homework: Unsend homework \'Fundamentals of Programming - updated\'.', '2019-03-28 13:21:49'),
(100, 7, 'Homework: Send homework \'Fundamentals of Programming - updated\'.', '2019-03-28 13:22:04'),
(101, 7, 'Homework: Unsend homework \'Fundamentals of Programming - updated\'.', '2019-03-28 13:28:46'),
(102, 7, 'Enroll Request: Approved enroll request.', '2019-03-29 12:29:05'),
(103, 7, 'Enroll Request: Approved enroll request.', '2019-03-29 12:34:17'),
(104, 7, 'Enroll Request: Approved enroll request.', '2019-03-29 12:39:28'),
(105, 7, 'Class List: Unenrolled student \'Samuel R. Greyjoy\'.', '2019-03-29 13:12:27'),
(106, 7, 'Class List: Enrolled student \'Samuel R. Greyjoy\'.', '2019-03-29 13:13:35'),
(107, 7, 'Class List: Unenroll student \'Samuel R. Greyjoy\'.', '2019-03-29 13:15:24'),
(108, 1, 'Faculty Load: Assigned new subject CSST 57-Roi Subject (4th year) to TID- 20488989 - Sam L. Smith', '2019-03-31 22:43:11'),
(109, 32, 'Homework: Submitted file on homework \'Fundamentals of programming (1)\'.', '2019-03-31 23:09:09'),
(110, 1, 'Faculty Load: Remove subject CSST 57-Roi Subject which is assigned to TID- 20488989 - Sam L. Smith.', '2019-04-01 02:09:38'),
(111, 1, 'Faculty Load: Assigned new subject CSST 57-Roi Subject (4th year) to TID- 20488989 - Sam L. Smith', '2019-04-01 02:14:27'),
(112, 1, 'Faculty Load: Remove subject CSST 57-Roi Subject which is assigned to TID- 20488989 - Sam L. Smith.', '2019-04-01 02:14:54'),
(113, 1, 'Curriculum: Removed subject \'Roi Subject\' from curriculum year 2017 (Course: Computer Science; Year Level: 4; Semester: 1st).', '2019-04-01 02:16:29'),
(114, 1, 'Faculty Load: Assigned new subject CSST134-Operating Systems (1st year) to TID- 20488989 - Sam L. Smith', '2019-04-01 02:17:17'),
(115, 1, 'Faculty Load: Remove subject CSST134-Operating Systems which is assigned to TID- 20488989 - Sam L. Smith.', '2019-04-01 02:17:40'),
(116, 1, 'Faculty Load: Assigned new subject CSST134-Operating Systems (1st year) to TID- 20488989 - Sam L. Smith', '2019-04-01 02:20:14'),
(117, 32, 'Homework: Submitted file on homework \'Fundamentals of programming (1)\'.', '2019-04-03 01:19:33'),
(118, 32, 'Homework: Submitted file on homework \'Fundamentals of programming (1)\'.', '2019-04-03 01:26:39'),
(119, 1, 'Settings: Set \'1st\' as default semester.', '2019-04-22 16:38:29'),
(120, 1, 'Settings: Set \'1st\' as default semester.', '2019-04-22 16:40:59'),
(121, 1, 'Settings: Set \'[object Object]\' as default semester.', '2019-04-22 16:48:23'),
(122, 1, 'Settings: Set \'[object Object]\' as default semester.', '2019-04-22 16:49:48'),
(123, 1, 'Settings: Set \'[object Object]\' as default semester.', '2019-04-22 16:59:31'),
(124, 1, 'Settings: Set \'[object Object]\' as default semester.', '2019-04-22 17:05:59'),
(125, 1, 'Settings: Set \'[object Object]\' as default semester.', '2019-04-22 17:06:20'),
(126, 1, 'Settings: Set \'[object Object]\' as default semester.', '2019-04-22 17:08:36'),
(127, 1, 'Settings: Set \'2017 - 2018\' as default school year.', '2019-04-28 00:10:26'),
(128, 1, 'Settings: Set \'1st\' as default semester.', '2019-04-28 01:10:45'),
(129, 1, 'Settings: Set \'2016 - 2017\' as default school year.', '2019-04-28 01:11:25'),
(130, 7, 'Class List: Enrolled student \'Samuel R. Greyjoy\'.', '2019-04-28 01:29:00'),
(131, 1, 'Curriculum: Assigned subject \'12459 - Computer Engineering\' to curriculum year 2017 (Course: Computer Science; Year Level: 2; Semester: 1st).', '2019-05-02 00:38:07'),
(132, 1, 'Semester: Save new semester 3rd.', '2019-05-02 00:40:08'),
(133, 1, 'Semester: Delete semester Summer.', '2019-05-02 00:40:19'),
(134, 1, 'Semester: Delete semester 3rd.', '2019-05-02 00:41:36'),
(135, 1, 'Semester: Save new semester Summer.', '2019-05-02 00:41:51'),
(136, 1, 'Curriculum: Assigned subject \'CSST 57 - Roi Subject\' to curriculum year 2017 (Course: Computer Science; Year Level: 3; Semester: Summer).', '2019-05-04 10:55:58');
...
(1, 'Amatong', 'Roi Larrence', 'Reyes', '695912563_523300636_816916693.jpg', '2017-02-01 04:00:41', '2019-03-24 09:01:28', b'1');
...
(64, 32, 30, 1, '2019-03-11 00:54:40'),
(68, 33, 31, 1, '2019-03-29 12:20:56'),
(69, 32, 12, 1, '2019-04-28 01:28:56');
...
(59, 35, 7, 'Hi Hello', '2019-03-23 10:27:17', NULL);
...
INSERT INTO `curriculum` (`curriculum_id`, `course_id`, `school_year_id`, `curriculum_year`) VALUES
(2, 2, 6, 2017),
(3, 2, 6, 2017),
(4, 2, 6, 2017),
(5, 2, 6, 2017),
...
(3, 'Lenten Season', '2019-04-13', '2019-04-16', 'green', '2017-04-16 13:33:22', '2019-03-10 07:13:34', 0),
...
(34, 32, 'Fundamentals of Programming - updated', '<ul><li><!--block-->Research about Fundamentals of programming</li><li><!--block-->Give example for each fundamentals of programming</li><li><!--block-->Run it on any text editor</li></ul>', NULL, '2019-03-29 00:00:00', NULL, '2019-03-24 05:18:33', 0);
...
(18, 34, 'file', 2015689, 'WMSU-ICS-Website.pptx', '907695137_592032278_478636012.pptx');
...
INSERT INTO `loads` (`load_id`, `curriculum_subject_id`, `course_id`, `subject_id`, `section`, `monday`, `tuesday`, `wednesday`, `thursday`, `friday`, `saturday`, `start_time`, `end_time`, `room`) VALUES
(27, 0, 2, 2, 'C', 1, 1, 1, 0, 0, 1, '09:30:00', '15:30:00', 'Ne29'),
(28, 0, 2, 6, 'A', 1, 0, 0, 0, 1, 1, '10:30:00', '12:30:00', 'ne23'),
(30, 0, 1, 3, 'B', 1, 0, 1, 0, 1, 0, '08:30:00', '10:00:00', 'NE29'),
(31, 0, 1, 13, 'D', 0, 0, 0, 1, 0, 1, '07:00:00', '10:30:00', 'NE31'),
(32, 0, 1, 3, 'D', 0, 0, 0, 1, 0, 1, '10:30:00', '12:30:00', 'AVR'),
(33, 0, 2, 4, 'C', 1, 0, 1, 0, 0, 0, '08:30:00', '10:00:00', 'AVR'),
(35, 0, 2, 2, 'A', 0, 1, 0, 1, 0, 0, '12:00:00', '13:30:00', 'NE25'),
(36, 0, 2, 2, 'A', 1, 0, 1, 0, 0, 0, '07:00:00', '08:30:00', 'NE28'),
(37, 4, 2, 2, 'A', 1, 1, 0, 0, 0, 0, '08:00:00', '10:30:00', 'NE29'),
(38, 3, 2, 4, 'B', 0, 0, 0, 1, 1, 0, '07:00:00', '10:00:00', 'AVR'),
(40, 25, 1, 3, 'P', 0, 1, 0, 0, 0, 0, '10:30:00', '12:00:00', 'TM 890'),
(44, 22, 1, 6, 'A', 1, 1, 0, 0, 0, 0, '10:00:00', '11:30:00', 'b12'),
(45, 22, 1, 6, NULL, 0, 0, 0, 0, 0, 0, NULL, NULL, NULL),
(46, 30, 2, 15, 'A', 1, 0, 1, 0, 0, 0, '10:30:00', '12:00:00', 'ne23'),
(50, 28, 2, 3, NULL, 0, 0, 0, 0, 0, 0, NULL, NULL, NULL);
...
INSERT INTO `messages` (`message_id`, `sender_id`, `send_to_id`, `message`, `sent_at`, `is_read`, `is_replied`, `reply_at`) VALUES
(3, 1, 8, '<div>dsdas dasdasdsadsad</div>', '2017-02-26 21:42:29', 0, 0, NULL),
(4, 1, 7, '<div>Sample message for Sir Joffrey.&nbsp;</div>', '2017-07-25 04:58:44', 0, 0, NULL),
(5, 1, 7, 'Hello', '2017-10-21 23:55:06', 0, 0, NULL),
(6, 1, 7, 'Hello Sample text', '2017-10-21 23:59:09', 0, 0, NULL),
(7, 7, 1, 'Hello Sample text', '2017-10-21 23:59:09', 0, 0, NULL),
(8, 7, 1, 'Hello Sample text', '2017-10-21 23:59:09', 0, 0, NULL),
(9, 1, 7, '<p>Hello</p>', '2017-11-01 05:38:37', 0, 0, NULL);
...
CREATE TABLE `notifications` (
  ...
  `notification_from_user_type` varchar(10) NOT NULL,
  `notification_to` int(11) NOT NULL,
  `notification_to_user_type` varchar(10) NOT NULL,
  `description` text NOT NULL,
  `path` text NOT NULL,
...
INSERT INTO `notifications` (`notification_id`, `notification_from`, `notification_from_user_type`, `notification_to`, `notification_to_user_type`, `description`, `path`, `datetime`, `is_read`) VALUES
(3, 1, 'admin', 7, 'teacher', 'Removed you as instructor from class CSST145-Thesis 1.5', '/teacher', '2019-03-28 12:37:23', 1),
(4, 1, 'admin', 13, 'student', 'You have been enrolled to class CSST1222-Advanced Database System (1st year)', '/student/subject/32/posts', '2019-03-28 12:47:02', 0),
(5, 1, 'admin', 32, 'student', 'You have been removed from class CSST1222-Advanced Database System (1st year)', '/student', '2019-03-28 12:54:31', 1),
(6, 7, 'teacher', 32, 'student', 'Assigned you a homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks/34', '2019-03-28 13:22:07', 0),
(7, 7, 'teacher', 22, 'student', 'Assigned you a homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks/34', '2019-03-28 13:22:07', 0),
(8, 7, 'teacher', 35, 'student', 'Assigned you a homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks/34', '2019-03-28 13:22:07', 0),
(9, 7, 'teacher', 13, 'student', 'Assigned you a homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks/34', '2019-03-28 13:22:07', 0),
(10, 7, 'teacher', 32, 'student', 'Remove homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks', '2019-03-28 13:28:47', 0),
(11, 7, 'teacher', 22, 'student', 'Remove homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks', '2019-03-28 13:28:48', 0),
(12, 7, 'teacher', 35, 'student', 'Remove homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks', '2019-03-28 13:28:48', 0),
(13, 7, 'teacher', 13, 'student', 'Remove homework entitled Fundamentals of Programming - updated', '/student/subject/32/homeworks', '2019-03-28 13:28:49', 0),
(14, 7, 'teacher', 36, 'student', 'Your request to enroll on class (CSST123) Data Analysis and Algorithm has been approved.', '/student/subject/33/posts', '2019-03-29 12:34:18', 0),
(15, 7, 'teacher', 36, 'student', 'Your request to enroll on class (CSST123) Data Analysis and Algorithm has been approved.', '/student/subject/33/posts', '2019-03-29 12:39:29', 0),
(16, 7, 'teacher', 13, 'student', 'You have been removed from class (CSST1222) Advanced Database System', '/student', '2019-03-29 13:12:29', 0),
(17, 7, 'teacher', 13, 'student', 'You have been enrolled to class (CSST1222) Advanced Database System', '/student/subject/32/posts', '2019-03-29 13:13:37', 0),
(18, 1, 'admin', 7, 'teacher', 'Assigned you as intructor to class CSST 57-Roi Subject (4th year)', '/teacher/subject/50/posts', '2019-03-31 22:43:10', 1),
(19, 32, 'student', 7, 'teacher', 'Submitted file on homework \'Fundamentals of programming (1)\'.', '/teacher/subject/32/homeworks/32', '2019-03-31 23:09:11', 1),
(20, 1, 'admin', 7, 'teacher', 'Removed you as instructor from class CSST 57-Roi Subject', '/teacher', '2019-04-01 02:09:39', 1),
(21, 1, 'admin', 7, 'teacher', 'Assigned you as intructor to class CSST 57-Roi Subject (4th year)', '/teacher/subject/51/posts', '2019-04-01 02:14:26', 1),
(22, 1, 'admin', 7, 'teacher', 'Removed you as instructor from class CSST 57-Roi Subject', '/teacher', '2019-04-01 02:14:56', 1),
(23, 1, 'admin', 7, 'teacher', 'Assigned you as intructor to class CSST134-Operating Systems (1st year)', '/teacher/subject/52/posts', '2019-04-01 02:17:16', 1),
(24, 1, 'admin', 7, 'teacher', 'Removed you as instructor from class CSST134-Operating Systems', '/teacher', '2019-04-01 02:17:41', 1),
(25, 1, 'admin', 7, 'teacher', 'Assigned you as intructor to class CSST134-Operating Systems (1st year)', '/teacher/subject/53/posts', '2019-04-01 02:20:13', 1),
(26, 32, 'student', 7, 'teacher', 'Submitted file on homework \'Fundamentals of programming (1)\'.', '/teacher/subject/32/homeworks/32', '2019-04-03 01:19:36', 0),
(27, 32, 'student', 7, 'teacher', 'Submitted file on homework \'Fundamentals of programming (1)\'.', '/teacher/subject/32/homeworks/32', '2019-04-03 01:26:41', 0),
(28, 7, 'teacher', 32, 'student', 'Disapproved your submitted answer for homework \'Fundamentals of programming (1)\'', '/student/subject/32/homeworks/32', '2019-04-03 12:40:06', 0),
(29, 7, 'teacher', 32, 'student', 'Approved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 12:57:41', 0),
(30, 7, 'teacher', 22, 'student', 'Approved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 12:57:44', 0),
(31, 7, 'teacher', 35, 'student', 'Approved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 12:57:46', 0),
(32, 7, 'teacher', 32, 'student', 'Disapproved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 13:00:33', 0),
(33, 7, 'teacher', 22, 'student', 'Disapproved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 13:00:33', 0),
(34, 7, 'teacher', 35, 'student', 'Disapproved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 13:00:33', 0),
(35, 7, 'teacher', 32, 'student', 'Approved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 13:01:12', 0),
(36, 7, 'teacher', 22, 'student', 'Approved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 13:01:12', 0),
(37, 7, 'teacher', 35, 'student', 'Approved your submitted answer for homework \'\'', '/student/subject/32/homeworks/32', '2019-04-03 13:01:12', 0),
(38, 7, 'teacher', 13, 'student', 'You have been enrolled to class (CSST1222) Advanced Database System', '/student/subject/32/posts', '2019-04-28 01:29:04', 0);
...
INSERT INTO `posts` (`post_id`, `class_id`, `account_id`, `description`, `post_at`, `updated_at`, `post_status`) VALUES
(3, 1, 10, '<div><!--block-->Danger alert preview. This alert is dismissable. A wonderful serenity has taken possession of my entire soul, like these sweet mornings of spring which I enjoy with my whole heart.Roi alrrenc esdsadsdssadasd</div>', '2017-03-05 11:33:09', '2017-03-06 04:42:52', 0),
(5, 1, 6, '<div><!--block-->Danger alert preview. This alert is dismissable. A wonderful serenity has taken possession of my entire soul, like these sweet mornings of spring which I enjoy with my whole heart.</div>', '2017-03-06 02:36:36', NULL, 0),
(6, 1, 6, '<div><!--block-->Danger alert preview. This alert is dismissable. A wonderful serenity has taken possession of my entire soul, like these sweet mornings of spring which I enjoy with my whole heart.Roi alrrenc esdsadsdssadasd</div>', '2017-03-06 02:37:59', '2017-03-06 04:27:30', 0),
(7, 8, 6, '<div><!--block-->hello everyone!!</div>', '2017-04-03 05:08:34', NULL, 0),
(8, 8, 6, '<div><!--block-->hey</div>', '2017-04-03 06:54:53', NULL, 0),
(10, 7, 7, '<div><!--block-->Hi Guys, may homework tayo? Thanks sa magrereply. :)</div>', '2017-04-16 12:49:09', NULL, 0),
(11, 9, 21, '<div><!--block-->Good day! Winter is coming.</div>', '2017-04-16 14:37:35', NULL, 0),
(12, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-14 16:48:58', NULL, 0),
(13, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:41:27', NULL, 0),
(14, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:41:56', NULL, 0),
(15, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:41:56', NULL, 0),
(16, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:41:56', NULL, 0),
(17, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:41:56', NULL, 0),
(18, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:03', NULL, 0),
(19, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:03', NULL, 0),
(20, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:03', NULL, 0),
(21, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:03', NULL, 0),
(22, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:09', NULL, 0),
(23, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:09', NULL, 0),
(24, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:09', NULL, 0),
(25, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:09', NULL, 0),
(26, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:15', NULL, 0),
(27, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:15', NULL, 0),
(28, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:15', NULL, 0),
(29, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:15', NULL, 0),
(30, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:26', NULL, 0),
(31, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:26', NULL, 0),
(32, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:26', NULL, 0),
(33, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:26', NULL, 0),
(35, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:32', NULL, 0),
(36, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:32', NULL, 0),
(37, 32, 7, '<div><!--block-->Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</div>', '2019-01-16 00:42:32', NULL, 0);
...
INSERT INTO `school_years` (`school_year_id`, `sy_from`, `sy_to`, `is_current_sy`) VALUES
(1, 2016, 2017, 1),
(6, 2017, 2018, 0),
(7, 2003, 2004, 0),
(15, 2007, 2008, 0);
...
INSERT INTO `semesters` (`semester_id`, `semester`, `is_current_semester`) VALUES
(1, '1st', 1),
(2, '2nd', 0),
(5, 'Summer', 0);
...
CREATE TABLE `students` (
  ...
  `image` text,
  `curriculum_year` int(11) NOT NULL DEFAULT '0',
  `created_at` datetime NOT NULL,
  `updated_at` datetime DEFAULT NULL,
  `is_active` bit(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `students` (`stud_id`, `student_id`, `last_name`, `first_name`, `middle_name`, `gender`, `birthdate`, `image`, `curriculum_year`, `created_at`, `updated_at`, `is_active`) VALUES
(1, 'SM201201808', 'Sali', 'Sansa', 'Gabica', 'Female', '2016-01-08', '', 0, '2017-02-03 23:34:49', '2017-04-03 06:35:16', b'1'),
(7, 'SM2012-01808', 'Ladiao', 'louise', 'Reyes', 'Male', '1995-09-14', '', 0, '2017-02-22 04:14:10', '2017-04-03 06:09:23', b'1'),
(11, 'SM2017-10909', 'Reyes', 'Cersie', 'Durant', 'Female', '1995-09-13', '', 0, '2017-02-26 10:34:39', '2017-04-03 06:40:43', b'1'),
(12, 'SM2012-018080', 'Greyjoy', 'Samuel', 'Reyes', 'Male', '1995-09-15', '', 0, '2017-03-25 05:35:39', '2017-04-03 06:57:02', b'1'),
(13, 'SM201201359', 'Stark', 'Sansa', 'Gabica', 'Female', '1995-09-16', NULL, 0, '2017-03-25 06:16:50', NULL, b'1'),
(14, 'SM201400001', 'Mormont', 'lyanna', 'Reyes', 'Female', '1969-10-10', '', 0, '2017-03-27 03:40:40', '2017-04-03 06:10:36', b'1'),
(20, 'SN2012-00230', 'Avanzado', 'Vincent', 'Raffy', 'Male', '1995-09-14', '225481330_920146245_660977176.jpg', 0, '2017-04-16 15:23:38', '2018-12-31 09:27:29', b'1'),
(22, 'SM2012-00120', 'Abduhari', 'Myra', 'Dandun', 'Female', '1995-06-27', '902178254_33241469_99782324.jpeg', 2016, '2017-04-16 16:04:13', '2019-03-10 07:33:39', b'0'),
(23, 'SM2012-00121', 'Parot', 'Mary Grace', 'Realm', 'Female', '1996-08-18', '', 0, '2017-04-16 16:15:16', NULL, b'1'),
(24, 'SM2012-00122', 'Kwan', 'Mark Dale', 'omelet', 'Male', '1994-09-25', '657736902_670447529_147536471.jpg', 0, '2017-04-16 16:23:35', '2019-01-13 05:39:34', b'1'),
(25, 'SM2012-00123', 'Sali', 'Alyssa', 'Rebolyos', 'Female', '1994-10-08', '', 0, '2017-04-16 16:27:09', NULL, b'1'),
(26, 'SM2012-00124', 'Locson', 'Crystal Jane', 'India', 'Female', '1995-08-08', '', 0, '2017-04-16 16:43:08', NULL, b'1'),
(27, 'SM-123', 'Yamato', 'Kira', 'Weasley', 'Male', '2018-12-06', '764104428_638283595_565294650.jpg', 0, '2019-01-01 00:10:08', NULL, b'1'),
(28, 'SM2018-001', 'Duterte', 'Jan Carlo', 'Solano', 'Female', '1994-01-21', '946373284_605978446_104258987.jpg', 0, '2019-01-09 13:00:37', '2019-01-23 16:23:03', b'1'),
(29, 'SM201400002', 'Paul', 'Chris', 'Reyes', 'Male', '2019-01-25', NULL, 0, '2019-01-10 17:39:30', NULL, b'1'),
(30, 'ZGMF-X20A', 'Abduhari', 'Myra', 'Dandun', 'Male', '2019-03-06', '479913589_24692452_77859801.jpeg', 2017, '2019-03-03 16:36:28', NULL, b'1'),
(31, 'SM2018-002', 'Zala', 'Athrun', 'Athrun', 'Male', '1993-03-11', '708708822_997733344_947753175.jpg', 2017, '2019-03-10 07:24:12', NULL, b'1');
...
INSERT INTO `subjects` (`subject_id`, `subject_code`, `subject_description`, `lec_units`, `lab_units`) VALUES
(2, 'CSST1222', 'Advanced Database System', 3, 1),
(3, 'CSST134', 'Operating Systems', 3, 1),
(4, 'CSST123', 'Data Analysis and Algorithm', 3, 0),
(6, 'CSST145', 'Thesis 1.5', 3, 2),
(7, 'CSST 132', 'Object Oriented Programming', 3, 1),
(13, 'CSST 57', 'Roi Subject', 1, 2),
(15, 'CSIT 189', 'Graphics and Design', 3, 3),
(16, '12456', 'New subject', 0, 0),
(19, '12459', 'Computer Engineering', 0, 0);
...
--
-- Dumping data for table `submitted_homeworks`
--

INSERT INTO `submitted_homeworks` (`submitted_homework_id`, `homework_id`, `student_id`, `is_submitted`, `date_submitted`, `approved_status`, `date_approved`) VALUES
(23, 25, 7, 0, NULL, 0, NULL),
(24, 25, 1, 0, NULL, 0, NULL),
(25, 25, 11, 0, NULL, 0, NULL),
(26, 23, 7, 1, '2017-03-15 23:38:27', 1, '2017-03-16 00:02:22'),
(27, 23, 1, 0, NULL, 0, NULL),
(28, 23, 11, 0, NULL, 0, NULL),
(29, 22, 7, 0, NULL, 0, NULL),
(30, 22, 1, 0, NULL, 0, NULL),
(31, 22, 11, 0, NULL, 0, NULL),
(32, 28, 7, 0, NULL, 0, NULL),
(33, 28, 1, 0, NULL, 0, NULL),
(34, 28, 11, 0, NULL, 0, NULL),
(77, 3, 11, 0, NULL, 0, NULL),
(99, 3, 7, 0, NULL, 0, NULL),
(102, 29, 11, 0, NULL, 0, NULL),
(103, 29, 7, 0, NULL, 0, NULL),
(104, 30, 13, 0, NULL, 0, NULL),
(105, 30, 7, 0, NULL, 0, NULL),
(106, 30, 14, 0, NULL, 0, NULL),
(107, 30, 12, 0, NULL, 0, NULL),
(108, 30, 16, 0, NULL, 0, NULL),
(109, 30, 19, 0, NULL, 0, NULL),
(110, 30, 17, 0, NULL, 0, NULL),
(111, 30, 18, 0, NULL, 0, NULL),
(112, 30, 22, 0, NULL, 0, NULL),
(113, 30, 23, 0, NULL, 0, NULL),
(114, 30, 24, 0, NULL, 0, NULL),
(115, 30, 25, 0, NULL, 0, NULL),
(116, 30, 26, 0, NULL, 0, NULL),
(117, 29, 22, 0, NULL, 0, NULL),
(118, 29, 20, 0, NULL, 0, NULL),
(119, 3, 22, 0, NULL, 0, NULL),
(120, 3, 20, 0, NULL, 0, NULL),
(121, 31, 11, 0, NULL, 0, NULL),
(122, 31, 7, 0, NULL, 0, NULL),
(123, 31, 22, 0, NULL, 0, NULL),
(124, 31, 20, 0, NULL, 0, NULL),
(125, 32, 28, 1, '2019-04-02 17:26:34', 1, '2019-04-03 05:00:56'),
(129, 32, 20, 0, NULL, 1, '2019-04-03 05:00:55'),
(148, 32, 30, 0, NULL, 1, '2019-04-03 05:00:54'),
(153, 34, 28, 1, '2019-03-24 11:40:57', 0, NULL),
(154, 34, 20, 0, NULL, 0, NULL),
(155, 34, 30, 0, NULL, 0, NULL),
(156, 34, 12, 0, NULL, 0, NULL),
(157, 32, 12, 0, NULL, 0, NULL);
...
--
-- Dumping data for table `submitted_homework_files`
--

INSERT INTO `submitted_homework_files` (`submitted_homework_file_id`, `submitted_homework_id`, `file_type`, `original_file_name`, `generated_file_name`, `size`) VALUES
(23, 23, NULL, NULL, NULL, 0),
(24, 24, NULL, NULL, NULL, 0),
(25, 25, NULL, NULL, NULL, 0),
(26, 26, 'file', 'testdb.mdb', '288177491_116210938_543731690.mdb', 0),
(27, 27, NULL, NULL, NULL, 0),
(28, 28, NULL, NULL, NULL, 0),
(29, 29, NULL, NULL, NULL, 0),
(30, 30, NULL, NULL, NULL, 0),
(31, 31, NULL, NULL, NULL, 0),
(32, 32, NULL, NULL, NULL, 0),
(33, 33, NULL, NULL, NULL, 0),
(34, 34, NULL, NULL, NULL, 0),
(118, 77, NULL, NULL, NULL, 0),
(142, 99, NULL, NULL, NULL, 0),
(145, 102, NULL, NULL, NULL, 0),
(146, 103, NULL, NULL, NULL, 0),
(147, 104, NULL, NULL, NULL, 0),
(148, 105, NULL, NULL, NULL, 0),
(149, 106, NULL, NULL, NULL, 0),
(150, 107, NULL, NULL, NULL, 0),
(151, 108, NULL, NULL, NULL, 0),
(152, 109, NULL, NULL, NULL, 0),
(153, 110, NULL, NULL, NULL, 0),
(154, 111, NULL, NULL, NULL, 0),
(155, 112, NULL, NULL, NULL, 0),
(156, 113, NULL, NULL, NULL, 0),
(157, 114, NULL, NULL, NULL, 0),
(158, 115, NULL, NULL, NULL, 0),
(159, 116, NULL, NULL, NULL, 0),
(160, 117, NULL, NULL, NULL, 0),
(161, 118, NULL, NULL, NULL, 0),
(162, 119, NULL, NULL, NULL, 0),
(163, 120, NULL, NULL, NULL, 0),
(164, 121, NULL, NULL, NULL, 0),
(165, 122, NULL, NULL, NULL, 0),
(166, 123, NULL, NULL, NULL, 0),
(167, 124, NULL, NULL, NULL, 0),
(168, 125, 'file', '16128081_1423839377640693_641125325_n (1).jpg', '107316609_779049194_397883187.jpg', 22629),
(172, 129, NULL, NULL, NULL, 0),
(181, 148, NULL, NULL, NULL, 0),
(186, 153, 'file', 'orca_share_media1550200161173.docx', '940602202_671215762_586476918.docx', 199686),
(187, 154, NULL, NULL, NULL, 0),
(188, 155, NULL, NULL, NULL, 0),
(189, 156, NULL, NULL, NULL, 0),
(190, 157, NULL, NULL, NULL, 0);
...
INSERT INTO `teachers` (`teacher_id`, `faculty_id`, `last_name`, `first_name`, `middle_name`, `gender`, `birthdate`, `academic_rank`, `designation`, `image`, `created_at`, `updated_at`, `is_active`) VALUES
(4, 'TID-2012-01808asd', 'Stark', 'Catelyn', 'Tully', 'Female', '1996-12-14 00:00:00', 'Masters', 'Department Head', '52093506_258056641_379913331.jfif', '2017-02-20 18:14:12', '2017-11-05 05:19:09', b'1'),
(5, 'TID- 20488989', 'Smith', 'Sam', 'Lannister', 'Male', '1989-08-27 00:00:00', 'Principal', 'Visiting Lecturer', '462499103_269778833_894287042.jpg', '2017-02-20 18:40:11', '2019-03-24 06:51:49', b'1'),
(6, 'TID-201230123', 'Thomas', 'Isiah', 'Thomas', 'Male', '1989-09-17 00:00:00', 'Masters', 'Department Head', '62683106_865447999_976196290.jpeg', '2017-02-21 05:51:44', '2017-02-21 08:12:55', b'1'),
(7, 'asdasd', 'asds', 'asd', 'asd', 'Male', '2017-01-31 00:00:00', 'ad', 'asd', '', '2017-02-22 04:15:33', NULL, b'1'),
(8, 'TID1234665', 'Lannister', 'Tyrion', 'Martell', 'Male', '2017-03-03 00:00:00', 'Masters', 'Adviser', '669262648_722226111_85741201.jpeg', '2017-03-05 13:02:14', '2017-11-19 10:30:49', b'1'),
(9, 'TID-201212345', 'Stark', 'Eddard', 'Ned', 'Male', '1887-01-30 00:00:00', 'Masters', 'Department Head - CS', '', '2017-04-16 14:19:37', '2017-04-16 14:20:38', b'1'),
(10, 'T-123', 'Amatong', 'Roi Larrence', 'Reyes', 'Male', '2018-12-24 00:00:00', 'Masters in Computer Science', 'Senior Web Dev', '365247707_566427159_956974259.jpg', '2018-12-31 21:42:37', '2019-03-10 08:11:47', b'1'),
(11, 'T-124', 'Harden', 'James', 'Harden', 'Male', '1990-12-01 00:00:00', 'Masters in Computer Science', 'Senior Web Dev', '294715490_209758226_960394192.jpg', '2018-12-31 23:38:53', NULL, b'1'),
(12, 'T-2019003', 'Duterte', 'JC', 'Solano', 'Male', '2019-01-21 00:00:00', 'Masters in Computer Science', 'Instructor 1', '', '2019-01-09 13:06:38', NULL, b'1'),
(13, 'T-2019004', 'Ramius', 'Marru', 'Ramius', 'Female', '1989-09-15 00:00:00', 'Masters in Computer Science', 'Senior Web Dev', '813326482_92719202_337869814.jpeg', '2019-03-10 07:44:28', NULL, b'1');
...
--
ALTER TABLE `activity_logs`
  MODIFY `activity_log_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=137;
...
--
ALTER TABLE `class_lists`
  MODIFY `class_list_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=70;
...
--
ALTER TABLE `curriculum`
  MODIFY `curriculum_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=42;

--
-- AUTO_INCREMENT for table `curriculum_subjects`
--
ALTER TABLE `curriculum_subjects`
  MODIFY `curriculum_subject_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=37;

--
-- AUTO_INCREMENT for table `curriculum_year_sem`
--
ALTER TABLE `curriculum_year_sem`
  MODIFY `curriculum_year_sem_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=41;
...
--
ALTER TABLE `homeworks`
  MODIFY `homework_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=35;

--
-- AUTO_INCREMENT for table `homework_uploaded_files`
--
ALTER TABLE `homework_uploaded_files`
  MODIFY `homework_uploaded_file_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;
...
--
ALTER TABLE `last_viewed_chats`
  MODIFY `LVC_ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;

--
-- AUTO_INCREMENT for table `loads`
--
ALTER TABLE `loads`
  MODIFY `load_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;
...
--
ALTER TABLE `students`
  MODIFY `stud_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=32;
...
--
ALTER TABLE `submitted_homeworks`
  MODIFY `submitted_homework_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=158;

--
-- AUTO_INCREMENT for table `submitted_homework_files`
--
ALTER TABLE `submitted_homework_files`
  MODIFY `submitted_homework_file_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=191;
