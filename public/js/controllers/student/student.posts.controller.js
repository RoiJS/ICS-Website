app.controller('studentPostsController', ['$scope', 'accountPostService', 'accountCommentsService', function ($scope, accountPostService, accountCommentsService) {
            ...
            if (result) {
                accountPostService.removePost(post_id).then((res) => {
                    if (res.data.status === 1) {
                        activityLogHelper.registerActivity(`Post: Deleted post.`).then(status => {
                            if (status) {
